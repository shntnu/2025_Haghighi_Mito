...
pixi run snakemake all_module --rulegraph 2>&1 | tail -n +2 > /tmp/rg_module.dot
pixi run snakemake all_module_diagnose --rulegraph 2>&1 | tail -n +2 > /tmp/rg_module_diagnose.dot
...
dot -Tpng /tmp/rg_module_diagnose.dot -o docs/pipeline/dag_module_diagnose_rules.png
...
pixi run snakemake all_module --dag 2>&1 | tail -n +2 > /tmp/dag_module.dot
pixi run snakemake all_module_diagnose --dag 2>&1 | tail -n +2 > /tmp/dag_module_diagnose.dot
...
dot -Tpng /tmp/dag_module_diagnose.dot -o docs/pipeline/dag_module_diagnose_jobs.png

# Cleanup temp files
rm /tmp/rg_baseline.dot /tmp/rg_notebook.dot /tmp/rg_module.dot /tmp/rg_module_diagnose.dot \
   /tmp/dag_baseline.dot /tmp/dag_notebook.dot /tmp/dag_module.dot /tmp/dag_module_diagnose.dot
...
echo "  METHOD 2 (Module - Clean Regenerated, Complete):"
echo "    - dag_module_rules.png (simplified - full pipeline)"
echo "    - dag_module_jobs.png (full - full pipeline)"
echo "    - dag_module_diagnose_rules.png (simplified - diagnostics only)"
echo "    - dag_module_diagnose_jobs.png (full - diagnostics only)"
