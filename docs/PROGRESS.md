...
- **Step 3 — Erin-style regeneration**: `scripts/regenerate_erin_suppfig3.py` (Snakemake rule `regenerate_erin_suppfig3`) ports Erin's upstream `plot_singlecell_cellsize.ipynb` (commit `ef8f26b`) cell-for-cell against the corrected CSV and our cached SQL parquet. QC gates produce the same sequence as Erin's executed notebook (72234 → 42659 → 41065 → 39017). Writes 5 PDFs (4 boxplots + 1 pairplot) under `data/interim/upstream_corrected/erin_suppfig3_corrected/` suitable as drop-in replacements for the published Supp Fig 3 panels. Only numeric difference vs Erin's published figure: subjects 272 and MCL004 are plotted in Control and SZA groups respectively (was SZA and MDD)
